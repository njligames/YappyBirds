...
local enter = function(self, scene)
    ...
    self.nodes[name] = imageNode.new(name, self.shared.geometry, self.spriteAtlas)
    ...
    menuScale = self.nodes[name]:shouldScale()
    ...
    if tbl[stateName] then
        for i=1, #tbl[stateName] do
            ...
            self.nodes[name] = pushButtonNode.new(name, self.shared.geometry, self.spriteAtlas, menuScale, fun, toggle)
            njli.World.getInstance():getScene():getRootNode():addChildNode(self.nodes[name].node)
