...
local testParticleEmitter = assert (loadfile(njli.ASSET_PATH("scripts/testParticleEmitter.lua")))
...
function labelEnglish()
    ...
    local pos = njli.btVector2(x, y+h*0.5)


    hud:fontFaceId(data["fontBold"])
    hud:fontSize(18)
    hud:fontFace("sans-bold")
    hud:fillColor(njli.WorldHUD.rgbaf(1.0,0,0,1.0))


    hud:textAlign(njli.JLI_HUD_ALIGN_MIDDLE)
...
local bg = hud:linearGradient(x,y+h*0.5,x+w*0.1,y+h, njli.WorldHUD.rgba(0,0,0,32), njli.WorldHUD.rgba(0,0,0,16));
hud:beginPath();
...
bg = hud:linearGradient(x,y+h*0.25,x+w*0.1,y+h, njli.WorldHUD.rgba(220,220,220,255), njli.WorldHUD.rgba(128,128,128,255));
hud:beginPath();
...
hud:fillColor(njli.WorldHUD.rgba(32,32,32,255));
hud:fill();
...
local gloss = hud:radialGradient(lx-ex*0.25,ly-ey*0.5, ex*0.1,ex*0.75, njli.WorldHUD.rgba(255,255,255,128), njli.WorldHUD.rgba(255,255,255,0));
hud:beginPath();
hud:ellipse(lx,ly, ex,ey);
hud:fillPaint(gloss);
hud:fill();

gloss = hud:radialGradient(rx-ex*0.25,ry-ey*0.5, ex*0.1,ex*0.75, njli.WorldHUD.rgba(255,255,255,128), njli.WorldHUD.rgba(255,255,255,0));
hud:beginPath();
hud:ellipse(rx,ry, ex,ey);
hud:fillPaint(gloss);
hud:fill();
...
function drawGraph(x, y, w, h, t)
    ...
    -- Graph background
    local bg = hud:linearGradient(x,y,x,y+h, njli.WorldHUD.rgba(0,160,192,0), njli.WorldHUD.rgba(0,160,192,64))
    hud:beginPath()
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,32))
    hud:strokeWidth(3.0)
    hud:stroke()
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,160,192,255))
    hud:strokeWidth(3.0)
    hud:stroke()

    -- Graph sample pos
    for i=0,5 do
        bg = hud:radialGradient(sx[i],sy[i]+2, 3.0, 8.0, njli.WorldHUD.rgba(0,0,0,32), njli.WorldHUD.rgba(0,0,0,0))
        hud:beginPath()
    ...
    hud:fillColor(njli.WorldHUD.rgba(0,160,192,255))
    hud:fill()
    hud:beginPath()
    for i=0,5 do
    hud:circle(sx[i], sy[i], 2.0)
    end
    hud:fillColor(njli.WorldHUD.rgba(220,220,220,255))
    hud:fill()
...
function drawLabel(text, x, y, w, h)
    ...
    hud:fontSize(18.0)
    hud:fontFace("sans")
    hud:fillColor(njli.WorldHUD.rgba(255,255,255,128))
...
function drawButton(preicon, text, x, y, w, h, col)
    ...
    local bg = hud:linearGradient(x,y,x,y+h, njli.WorldHUD.rgba(255,255,255,param), njli.WorldHUD.rgba(0,0,0,param))
    hud:beginPath()
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,48))
    hud:stroke()

    local rect = njli.Rect()
    hud:fontSize(20.0)
    hud:fontFace("sans-bold")
    ...
    if (preicon ~= 0) then
        hud:fontSize(h*1.3)
        hud:fontFace("icons")
        iw = hud:textBounds(0,0, rect, njli.WorldHUD.cpToUTF8(preicon))
        iw = iw + h*0.15
    end
    ...
    if (preicon ~= 0) then
        hud:fontSize(h*1.3)
        hud:fontFace("icons")
        hud:fillColor(njli.WorldHUD.rgba(255,255,255,96))

        local align = bit.bor((njli.JLI_HUD_ALIGN_LEFT), (njli.JLI_HUD_ALIGN_MIDDLE))
        hud:textAlign(align)
        hud:text(x+w*0.5-tw*0.5-iw*0.75, y+h*0.5, njli.WorldHUD.cpToUTF8(preicon))
    end
    ...
    local align = bit.bor((njli.JLI_HUD_ALIGN_LEFT), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
    hud:fillColor(njli.WorldHUD.rgba(0,0,0,160))
    hud:text(x+w*0.5-tw*0.5+iw*0.25, y+h*0.5-1,text, NULL)
    hud:fillColor(njli.WorldHUD.rgba(255,255,255,160))
...
function drawColorwheel(x, y, w, h, t)
    ...
    for i=0,5 do
        local a0 = i / 6.0 * njli.PI * 2.0 - aeps
        local a1 = (i+1.0) / 6.0 * njli.PI * 2.0 + aeps
        hud:beginPath()
        hud:arc(cx,cy, r0, a0, a1, njli.JLI_HUD_WINDING_CW)
        hud:arc(cx,cy, r1, a1, a0, njli.JLI_HUD_WINDING_CCW)
        hud:closePath()
        ...
        local paint = hud:linearGradient(ax,ay, bx,by, njli.WorldHUD.hsla(a0/(njli.PI*2),1.0,0.55,255), njli.WorldHUD.hsla(a1/(njli.PI*2),1.0,0.55,255))
        hud:fillPaint(paint)
        hud:fill()
    end
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,64))
    hud:strokeWidth(1.0)
    hud:stroke()
    ...
    hud:save()
    hud:translate(cx,cy)
    hud:rotate(hue*njli.PI*2)

    -- Marker on
    hud:strokeWidth(2.0)
    hud:beginPath()
    hud:rect(r0-1,-3,r1-r0+2,6)
    hud:strokeColor(njli.WorldHUD.rgba(255,255,255,192))
    hud:stroke()

    paint = hud:boxGradient(r0-3,-5,r1-r0+6,10, 2,4, njli.WorldHUD.rgba(0,0,0,128), njli.WorldHUD.rgba(0,0,0,0))
    hud:beginPath()
    hud:rect(r0-2-10,-4-10,r1-r0+4+20,8+20)
    hud:rect(r0-2,-4,r1-r0+4,8)
    hud:pathWinding(njli.JLI_HUD_SOLIDITY_HOLE)
    hud:fillPaint(paint)
    hud:fill()

    -- Center triangle
    local r = r0 - 6
    ax = math.cos(120.0/180.0*njli.PI) * r
    ay = math.sin(120.0/180.0*njli.PI) * r
    bx = math.cos(-120.0/180.0*njli.PI) * r
    by = math.sin(-120.0/180.0*njli.PI) * r
    hud:beginPath()
    hud:moveTo(r,0)
    hud:lineTo(ax,ay)
    hud:lineTo(bx,by)
    hud:closePath()
    paint = hud:linearGradient(r,0, ax,ay, njli.WorldHUD.hsla(hue,1.0,0.5,255), njli.WorldHUD.rgba(255,255,255,255))
    hud:fillPaint(paint)
    hud:fill()
    paint = hud:linearGradient((r+ax)*0.5, (0+ay)*0.5, bx,by, njli.WorldHUD.rgba(0,0,0,0), njli.WorldHUD.rgba(0,0,0,255))
    hud:fillPaint(paint)
    hud:fill()
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,64))
    hud:stroke()
    ...
    ax = math.cos(120.0/180.0*njli.PI) * r*0.3
    ay = math.sin(120.0/180.0*njli.PI) * r*0.4
    hud:strokeWidth(2.0)
    hud:beginPath()
    hud:circle(ax,ay,5)
    hud:strokeColor(njli.WorldHUD.rgba(255,255,255,192))
    ...
    paint = hud:radialGradient(ax,ay, 7,9, njli.WorldHUD.rgba(0,0,0,64), njli.WorldHUD.rgba(0,0,0,0))
    hud:beginPath()
...
function drawLines(x, y, w, h, t)
    ...
    joins[0] = njli.JLI_HUD_LINECAP_MITER
    joins[1] = njli.JLI_HUD_LINECAP_ROUND
    joins[2] = njli.JLI_HUD_LINECAP_BEVEL

    caps[0] = njli.JLI_HUD_LINECAP_BUTT
    caps[1] = njli.JLI_HUD_LINECAP_ROUND
    caps[2] = njli.JLI_HUD_LINECAP_SQUARE
    ...
    for i=0,2 do
        for j=0,2 do
            ...
            hud:strokeColor(njli.WorldHUD.rgba(0,0,0,160))
            hud:beginPath()
            ...
            hud:lineCap(njli.JLI_HUD_LINECAP_BUTT)
            hud:lineJoin(njli.JLI_HUD_LINECAP_BEVEL)

            hud:strokeWidth(1.0)
            hud:strokeColor(njli.WorldHUD.rgba(0,192,255,255))
            hud:beginPath()
            hud:moveTo(fx+pts[0], fy+pts[1])
            hud:lineTo(fx+pts[2], fy+pts[3])
            hud:lineTo(fx+pts[4], fy+pts[5])
...
function drawWindow(title, x, y, w, h)
    ...
    hud:fillColor(njli.WorldHUD.rgba(28,30,34,192))
    --	hud:fillColor(njli.WorldHUD.rgba(0,0,0,128))
    hud:fill()
    
    -- Drop shadow
    local shadowPaint = hud:boxGradient(x,y+2, w,h, cornerRadius*2, 10, njli.WorldHUD.rgba(0,0,0,128), njli.WorldHUD.rgba(0,0,0,0))
    hud:beginPath()
    ...
    -- Header
    local headerPaint = hud:linearGradient(x,y,x,y+15, njli.WorldHUD.rgba(255,255,255,8), njli.WorldHUD.rgba(0,0,0,16))
    hud:beginPath()
    ...
    hud:fontSize(18.0)
    hud:fontFace("sans-bold")
    hud:textAlign(njli.JLI_HUD_ALIGN_CENTER)
    ...
    hud:fontBlur(0)
    hud:fillColor(njli.WorldHUD.rgba(220,220,220,160))
...
function drawSearchBox(text, x, y, w, h)
    ...
    -- Edit
    local bg = hud:boxGradient(x,y+1.5, w,h, h/2,5, njli.WorldHUD.rgba(0,0,0,16), njli.WorldHUD.rgba(0,0,0,92))
    hud:beginPath()
    hud:roundedRect(x,y, w,h, cornerRadius)
    hud:fillPaint(bg)
    hud:fill()
    ...
    hud:fontSize(h*1.3)
    hud:fontFace("icons")
    hud:fillColor(njli.WorldHUD.rgba(255,255,255,64))

    local align = bit.bor((njli.JLI_HUD_ALIGN_CENTER), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
    hud:text(x+h*0.55, y+h*0.55, njli.WorldHUD.cpToUTF8(ICON_SEARCH))
    
    hud:fontSize(20.0)
    hud:fontFace("sans")
    hud:fillColor(njli.WorldHUD.rgba(255,255,255,32))
    ...
    local align = bit.bor((njli.JLI_HUD_ALIGN_CENTER), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
    hud:text(x+w-h*0.55, y+h*0.55, njli.WorldHUD.cpToUTF8(ICON_CIRCLED_CROSS))
end
...
function drawDropDown(text, x, y, w, h)
    ...
    local bg = hud:linearGradient(x,y,x,y+h, njli.WorldHUD.rgba(255,255,255,16), njli.WorldHUD.rgba(0,0,0,16))
    hud:beginPath()
    hud:roundedRect(x+1,y+1, w-2,h-2, cornerRadius-1)
    hud:fillPaint(bg)
    hud:fill()
    ...
    local align = bit.bor((njli.JLI_HUD_ALIGN_CENTER), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
    hud:text(x+w-h*0.5, y+h*0.5, njli.WorldHUD.cpToUTF8(ICON_CHEVRON_RIGHT))
end
...
function drawEditBoxBase(x, y, w, h)
    -- Edit
    local bg = hud:boxGradient(x+1,y+1+1.5, w-2,h-2, 3,4, njli.WorldHUD.rgba(255,255,255,32), njli.WorldHUD.rgba(32,32,32,32))
    hud:beginPath()
...
function drawEditBoxNum(text, units, x, y, w, h)
    ...
    local align = bit.bor((njli.JLI_HUD_ALIGN_RIGHT), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
...
function drawCheckBox(text, x, y, w, h)
    ...
    local bg = hud:boxGradient(x+1,y+(h*0.5)-9+1, 18,18, 3,3, njli.WorldHUD.rgba(0,0,0,32), njli.WorldHUD.rgba(0,0,0,92))
    hud:beginPath()
    ...
    local align = bit.bor((njli.JLI_HUD_ALIGN_CENTER), (njli.JLI_HUD_ALIGN_MIDDLE))
    hud:textAlign(align)
    hud:text(x+9+2, y+h*0.5, njli.WorldHUD.cpToUTF8(ICON_CHECK))
end
...
function drawSlider(pos, x, y, w, h)
    ...
    -- Slot
    local bg = hud:boxGradient(x,cy-2+1, w,4, 2,2, njli.WorldHUD.rgba(0,0,0,32), njli.WorldHUD.rgba(0,0,0,128))
    hud:beginPath()
    ...
    -- Knob Shadow
    bg = hud:radialGradient(x+(pos*w),cy+1, kr-3,kr+3, njli.WorldHUD.rgba(0,0,0,64), njli.WorldHUD.rgba(0,0,0,0))
    hud:beginPath()
    ...
    -- Knob
    local knob = hud:linearGradient(x,cy-kr,x,cy+kr, njli.WorldHUD.rgba(255,255,255,16), njli.WorldHUD.rgba(0,0,0,16))
    hud:beginPath()
    hud:circle(x+(pos*w),cy, kr-1)
    hud:fillColor(njli.WorldHUD.rgba(40,43,48,255))
    hud:fill()
    hud:fillPaint(knob)
    hud:fill()
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,92))
    hud:stroke()
...
function drawSpinner(cx, cy, r, t)
    ...
    local bx = cx + math.cos(a1) * (r0+r1)*0.5
    local by = cy + math.sin(a1) * (r0+r1)*0.5
    local paint = hud:linearGradient(ax,ay, bx,by, njli.WorldHUD.rgba(0,0,0,0), njli.WorldHUD.rgba(0,0,0,128))
    hud:fillPaint(paint)
    hud:fill()
...
function drawThumbnails(x, y, w, h, images, nimages, t)
    ...
    -- Drop shadow
    local shadowPaint = hud:boxGradient(x,y+4, w,h, cornerRadius*2, 20, njli.WorldHUD.rgba(0,0,0,128), njli.WorldHUD.rgba(0,0,0,0))
    hud:beginPath()
    hud:rect(x-10,y-10, w+20,h+30)
    hud:roundedRect(x,y, w,h, cornerRadius)
    hud:pathWinding(njli.JLI_HUD_SOLIDITY_HOLE)
    ...
    hud:lineTo(x+1,y+arry+11)
    hud:fillColor(njli.WorldHUD.rgba(200,200,200,255))
    hud:fill()
    ...
    for i=0,(nimages-1) do
        ...
        local imgPaint = hud:imagePattern(tx+ix, ty+iy, iw,ih, 0.0/180.0*njli.PI, images[i], a)
        hud:beginPath()
        hud:roundedRect(tx,ty, thumb,thumb, 5)
        hud:fillPaint(imgPaint)
        hud:fill()
        
        shadowPaint = hud:boxGradient(tx-1,ty, thumb+2,thumb+2, 5, 3, njli.WorldHUD.rgba(0,0,0,128), njli.WorldHUD.rgba(0,0,0,0))
        hud:beginPath()
    ...
    -- Hide fades
    local fadePaint = hud:linearGradient(x,y,x,y+6, njli.WorldHUD.rgba(200,200,200,255), njli.WorldHUD.rgba(200,200,200,0))
    hud:beginPath()
    hud:rect(x+4,y,w-8,6)
    hud:fillPaint(fadePaint)
    hud:fill()

    fadePaint = hud:linearGradient(x,y+h,x,y+h-6, njli.WorldHUD.rgba(200,200,200,255), njli.WorldHUD.rgba(200,200,200,0))
    hud:beginPath()
    ...
    -- Scroll bar
    shadowPaint = hud:boxGradient(x+w-12+1,y+4+1, 8,h-8, 3,4, njli.WorldHUD.rgba(0,0,0,32), njli.WorldHUD.rgba(0,0,0,92))
    hud:beginPath()
    hud:roundedRect(x+w-12,y+4, 8,h-8, 3)
    hud:fillPaint(shadowPaint)
    --	hud:fillColor(njli.WorldHUD.rgba(255,0,0,128))
    hud:fill()

    local scrollh = (h/stackh) * (h-8)
    shadowPaint = hud:boxGradient(x+w-12-1,y+4+(h-8-scrollh)*u-1, 8,scrollh, 3,4, njli.WorldHUD.rgba(220,220,220,255), njli.WorldHUD.rgba(128,128,128,255))
    hud:beginPath()
...
function drawWidths(x, y, width)
    ...
    hud:strokeColor(njli.WorldHUD.rgba(0,0,0,255))
...
function drawScissor(x, y, t)
    ...
    hud:translate(x, y)
    hud:rotate(njli.WorldHUD.degToRad(5))
    hud:beginPath()
    hud:rect(-20,-20,60,40)
    hud:fillColor(njli.WorldHUD.rgba(255,0,0,255))
    hud:fill()
    ...
    hud:fillColor(njli.WorldHUD.rgba(255,128,0,64))
    hud:fill()
    hud:restore()
    ...
    hud:beginPath()
    hud:rect(-20,-10,60,30)
    hud:fillColor(njli.WorldHUD.rgba(255,128,0,255))
    hud:fill()
...
while ((nrows = hud:textBreakLines(start, end, width, rows, 3))) {
    for (i = 0 i < nrows i++) {
        ...
        hud:beginPath()
        hud:fillColor(njli.WorldHUD.rgba(255,255,255,hit?64:16))
        hud:rect(x, y, row->width, lineh)
        hud:fill()
        
        hud:fillColor(njli.WorldHUD.rgba(255,255,255,255))
        ...
        if (hit) {
            ...
            hud:beginPath()
            hud:fillColor(njli.WorldHUD.rgba(255,192,0,255))
...
hud:fillColor(njli.WorldHUD.rgba(0,0,0,220))
...
function RenderHUD()
    ...
    local mx, my, width, height, t, blowup = njli.World.getInstance():getViewportDimensions():x(), njli.World.getInstance():getViewportDimensions():y(), njli.World.getInstance():getViewportDimensions():x(), njli.World.getInstance():getViewportDimensions():y(), njli.World.getInstance():getWorldClock():totalMilliseconds(), 0
    ...
    hud:save()
    if (blowup==1) then
        hud:rotate(math.sin(t*0.3)*5.0/180.0*njli.PI)
        hud:scale(2.0, 2.0)
    end
    ...
    drawButton(ICON_LOGIN, "Sign in", x+138, y, 140, 28, njli.WorldHUD.rgba(0,96,128,255))
    y = y + 45
    ...
    drawButton(ICON_TRASH, "Delete", x, y, 160, 28, njli.WorldHUD.rgba(128,16,8,255))
    drawButton(0, "Cancel", x+170, y, 110, 28, njli.WorldHUD.rgba(0,0,0,0))
